...
function Section({ title, palette, headingStyle, children }: SectionProps): React.ReactElement {
  return (
    <div className="px-4 py-5 sm:px-10 sm:py-8" style={{ borderTop: `1px solid ${palette.border}` }}>
      <h2 className="mb-4 text-lg sm:mb-5 sm:text-xl" style={{ ...headingStyle, color: palette.text }}>
        {title}
      </h2>
      {children}
    </div>
...
export function BrandGuide(): React.ReactElement {
  ...
  return (
    ...
    >
      {/* ─── HEADER ─── */}
      <div
        className="rounded-t-xl px-4 py-6 text-center sm:px-10 sm:py-12"
      ...
      <Section title="Wordmark" palette={palette} headingStyle={headingStyle}>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {/* On light bg */}
          <div
            className="flex flex-col items-center justify-center rounded-lg p-5 sm:p-8"
          ...
          >
            <span className="text-2xl" style={{ ...headingStyle, color: palette.onPrimary }}>
              {name}
            </span>
      ...
      <Section title="Brand Colors" palette={palette} headingStyle={headingStyle}>
        ...
        >
          <div
            className="flex h-20 items-end p-4 sm:h-24 sm:p-5"
          ...
          {isGradient && gradientStops !== null && (
            <div className="flex flex-col sm:flex-row">
              {gradientStops.map((stop: string, i: number) => (
                <div
                  key={i}
                  className="flex h-6 flex-1 items-center justify-center sm:h-8"
        ...
        <div className="space-y-3 sm:space-y-5">
          ...
          >
            <div className="mb-2">
              ...
              <span className="ml-2 font-mono text-xs" style={{ color: palette.textMuted }}>
      ...
      <Section title="Buttons" palette={palette} headingStyle={headingStyle}>
        ...
        <div
          className="flex flex-wrap gap-3 rounded-lg p-4 sm:p-5"
          style={{ backgroundColor: lightPalette.bg, border: `1px solid ${lightPalette.border}` }}
        >
          <div
          ...
          <div
            className="rounded-lg px-5 py-2.5 text-sm"
            style={{ ...bodyStyle, backgroundColor: palette.secondary, color: palette.onSecondary, fontWeight: 600 }}
          >
            Secondary
          </div>
          <div
        ...
        >
          ...
          <div
            className="rounded-lg px-5 py-2.5 text-sm"
            style={{ ...bodyStyle, backgroundColor: darkPalette.secondary ?? palette.secondary, color: darkPalette.onSecondary ?? palette.onSecondary, fontWeight: 600 }}
          >
            Secondary
          </div>
          <div
      ...
      {tone !== undefined && (
        <Section title="Voice & Tone" palette={palette} headingStyle={headingStyle}>
          ...
          {/* Tone context — stacked cards on mobile, table on sm+ */}
          <div
            ...
            <table className="w-full text-sm" style={bodyStyle}>
              <thead>
                <tr style={{ backgroundColor: palette.surface }}>
                  <th className="px-4 py-2.5 text-left text-xs font-semibold" style={{ color: palette.textMuted }}>Context</th>
                  <th className="px-4 py-2.5 text-left text-xs font-semibold" style={{ color: palette.textMuted }}>Tone</th>
                  <th className="px-4 py-2.5 text-left text-xs font-semibold" style={{ color: palette.textMuted }}>Example</th>
                </tr>
              </thead>
              <tbody>
                {tone.toneTable.map((row: { context: string; tone: string; example: string }, i: number) => (
                  <tr key={i} style={{ borderTop: `1px solid ${palette.border}` }}>
                    <td className="px-4 py-2.5" style={{ color: palette.text }}>{row.context}</td>
                    <td className="px-4 py-2.5" style={{ color: palette.textMuted }}>{row.tone}</td>
                    <td className="px-4 py-2.5 italic" style={{ color: palette.textMuted }}>{row.example}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {tone.toneTable.map((row: { context: string; tone: string; example: string }, i: number) => (
              ...
              >
                <div className="flex items-baseline justify-between">
                  <span className="text-sm font-medium" style={{ ...bodyStyle, color: palette.text }}>{row.context}</span>
                  <span className="text-xs" style={{ ...bodyStyle, color: palette.textMuted }}>{row.tone}</span>
                </div>
                <p className="mt-1.5 text-xs italic leading-relaxed" style={{ ...bodyStyle, color: palette.textMuted }}>
                  {row.example}
                </p>
              </div>
            ))}
      ...
      {/* Footer */}
      <div className="px-4 py-5 text-center sm:px-10 sm:py-6">

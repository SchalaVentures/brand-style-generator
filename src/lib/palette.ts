...
function buildLightPalette(
  ...
  // Semantic — blend with brand hue (5% influence)
  const successHue: number = blendHue(145, hue)
  const warningHue: number = blendHue(60, hue)
  ...
  return {
    ...
    accent: accentHex,
    onPrimary: contrastRatio(primaryHex, '#FFFFFF') >= 3 ? '#FFFFFF' : '#111111',
    onSecondary: contrastRatio(secondaryHex, '#FFFFFF') >= 3 ? '#FFFFFF' : '#111111',
    onTertiary: contrastRatio(tertiaryHex, '#FFFFFF') >= 3 ? '#FFFFFF' : '#111111',
    onAccent: contrastRatio(accentHex, '#FFFFFF') >= 3 ? '#FFFFFF' : '#111111',
    bg,
    surface,
    surfaceRaised,
    border,
    text,
    textMuted,
    success: oklchToHex(0.63, 0.18, successHue),
    warning: oklchToHex(0.69, 0.19, warningHue),
    error: oklchToHex(0.55, 0.20, errorHue),
  }
}
...
function buildDarkPalette(
  ...
  return {
    ...
    warning: oklchToHex(0.76, 0.18, warningHue),
...
function blendHue(semanticHue: number, brandHue: number): number {
  return normalizeHue(semanticHue * 0.95 + brandHue * 0.05)
}
